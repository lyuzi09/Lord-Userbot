...
async def fastpurger(purg):
    ...
    if purg.reply_to_msg_id is not None:
        ...
        return await purg.edit("`Mohon Balas Ke Pesan Master ツ `")
    ...
    if BOTLOG:
        await purg.client.send_message(
            BOTLOG_CHATID,
            "Berhasil Menghapus Pesan Master " + str(count) + " Pesan Berhasil  Dibersihkan.")
    """
    await sleep(2)
    await done.delete()
...
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Berhasil Menghapus Pesan Master,` " + str(count) + " `Pesan Telah Dihapus ツ`",
    )
    """
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`Master Telah Menghapus Pesan,` " + str(count) + " Pesan Telah Dihapus ツ`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
...
async def delete_it(delme):
    ...
    if delme.reply_to_msg_id:
        try:
            ...
            if BOTLOG:
                await delme.client.send_message(
                    BOTLOG_CHATID, "`Master Berhasil Menghapus Pesan ツ`")
            """
